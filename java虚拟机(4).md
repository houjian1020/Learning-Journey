...
2、说一下 JVM由那些部分组成？(重点)
    ...
    JVM包含两个子系统和两个组件:
`       Class loader(类加载器)子系统：将 .class 文件加载到内存的方法区中
        Runtime data area(运行时数据区域)组件：这就是我们常说的JVM的内存
        Execution engine（执行引擎）子系统：负责把字节码翻译成机器指令并执行。
        Native Interface(本地接口)组件：调用底层操作系统或 C/C++ 编写的本地方法。
        `


Java 代码的运行流程(重点)
`   编译阶段：编译器把 Java 代码转换成字节码(calss文件)
    类加载阶段：类加载器（ClassLoader）再把字节码加载到 运行时数据区（Runtime data area）的 方法区的内存中
    链接与初始化阶段：验证、准备、解析和初始化四个步骤，负责确保字节码安全、分配静态变量内存并完成赋值，最终使类达到可执行状态
    执行阶段： 执行引擎（Execution Engine），将字节码转换为机器指令执行，并在运行时协同内存区进行对象分配、方法调用及垃圾回收`


3、说一下 JVM 运行时数据区(重点)
`   分类：线程共享区域（随 JVM 启动而创建，随 JVM 退出而销毁； 包含：堆 方法区）   线程私有区域（随线程创建而创建，随线程结束而销毁，无需 GC； 包含：栈 方法栈 程序计数器）
    ...
    Java 堆（Java Heap）：Java 虚拟机中内存最大的一块，是被所有线程共享的，几乎所有的对象实例都在这里分配内存，垃圾回收器（GC）管理的主要区域；
...
9、你能给我详细的介绍Java堆吗?(重点)
    `定义：虚拟机启动时创建、存放对象实例和数组、线程共享、占用内存最大、GC核心区域、内存可变、分区存储（新生代、老年代）`
    特点：
        动态分配与回收：对象内存的分配和回收由 JVM 自动管理，
        大小可变：堆内存大小可通过 JVM 启动参数动态调整（-Xmx‌：堆内存允许扩张的最大值  -Xms‌：堆内存初始大小）
        垃圾收集‌：堆是垃圾收集器的主要工作区域（也被成为“GC堆”）
        分代收集：新生代存储新创建的对象；老生代存储生命周期较长的对象
        结构：新生代（eden/from/to）、老年代（多次GC后的对象）
        线程共享
...
12、什么是JVM字节码执行引擎
`   概述：负责把字节码翻译成机器指令并执行
    解释器（Interpreter）：逐行将字节码翻译成机器码执行，启动快，但执行慢。
    即时编译器（JIT Compiler）：将热点代码（频繁执行的代码）整体编译成机器码并缓存，执行极快。
    垃圾回收器（Garbage Collector）：自动管理堆内存，回收不再使用的对象`
...
16、java会存在内存泄漏吗？请说明为什么？MemoryLeakExample.java(重点)
`   答案：存在着内存泄漏;堆内存设置过小、静态集合类滥用、资源未正常关闭 等 引用未被正常释放的情况
    定义:业务上已废弃，但引用链未断裂的对象，都是内存泄漏`
...
22、Full GC是什么
    `清理的是整个 Java 堆空间（年轻代 + 老年代），以及元空间（Metaspace）中的无用类元数据`


~~23、JVM中的永久代中会发生垃圾回收吗
    垃圾回收不会发生在永久代，如果永久代满了或者是超过了临界值，会触发完全垃圾回收(Full GC)~~

24、讲一下新生代、老年代、元空间的区别 (重点)
...
`    新生代：采用复制算法进行回收
    ...
    老年代：存放经过多次（默认15次）新生代GC仍然存活的对象；采用“标记-清理”或“标记-整理”算法进行回收
    元空间：JDK 8 及以后，永久代被“元空间”替代，元空间使用的是本地物理内存（Native Memory），不再受 Java 堆大小的限制`
    

25、Minor GC、Major GC、Full GC是什么
`   Minor GC：新生代GC，Minor GC非常频繁，回收速度也比较快。（复制算法）
    Major GC：老年代GC，Major GC的速度要比Minor GC慢的多。（标记-清除/标记-整理算法）
    Full GC：内存大扫除，包括堆空间（新生代、老年代）和元空间（方法区）`

26、Minor GC、Major GC、Full GC区别及触发条件
   ` Minor GC：eden区满时
    Major GC：老年代空间不足
    Full GC：老年代空间不足、元空间空间不足、System.gc()调用`
...
29、类加载的机制及过程  ClassLoaderExample.java (重点)
    机制：Java类加载器遵循一个称为`双亲委派模型`的机制。当一个类加载器需要加载一个类时，它首先将这个任务委托给它的父类加载器。如果父类加载器找不到该类，子类加载器才会尝试自己加载。
...
34、`arthas(阿尔萨斯)监控`
接入方式：   非侵入式动态 Attach（（推荐：线上应急排查））、  Java Agent 启动（推荐：容器化/云原生环境）  Web 控制台（推荐：集中管理/图形化操作）
常用命令：
    dashboard	实时查看 JVM 全局状态，快速判断 CPU、内存、线程、GC 是否异常
    trace	追踪`方法`内部调用链路及耗时	定位接口慢、性能瓶颈
    watch	实时观测`方法`的入参、返回值、异常	排查参数异常、业务逻辑错误
    thread	查看`线程`状态和堆栈	排查线程死锁、CPU 飙高
    profiler	生成 CPU/内存火焰图	深度性能分析，定位热点方法
    jad	反编译线上已加载的类	确认线上代码版本是否与预期一致
