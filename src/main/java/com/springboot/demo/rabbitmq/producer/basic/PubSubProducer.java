...
/**
 * 发布订阅模式：一个生产者=====>多个队列（每个队列接收所有消息）=======>多个消费者
 * 特点：每条消息都会发送到所有队列
 * 步骤：创建连接工厂————设置连接参数————创建连接————创建管道————声明交换机————声明队列————交换机与队列绑定————发送消息————关闭资源
 */
public class PubSubProducer {
    // 队列名称
    private final static String QUEUE_NAME1 = "pubsub_queue1";
    private final static String QUEUE_NAME2 = "pubsub_queue2";
    // 交换机名称
...
//        factory.setPassword("guest");// 设置RabbitMQ密码，默认为"guest"
        ...
        try {
             ...
             * body：发送给消费者的消息
             */
            String message = "PubSub, RabbitMQ!";
