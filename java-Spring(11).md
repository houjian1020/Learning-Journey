...
`   
    本质：bean查找 + 属性注入
    bean查找：扫描所有被 @Component、@Controller、@Service、@Repository、@Bean 标记的类，生成 Bean 定义
    属性注入：解析被@Autowired修饰的属性，进行注入
    三种注入方式：	@Autowired加在字段、构造器、Setter上
`
...
`   @Autowired和@Resource可用于：构造函数、成员变量、Setter方法
    ...
    注意：类型具有多个bean指：接口多实现       按照名称注入指：@Resource("AuditOrgaAreaServiceImpl")
    多个bean解决方案：@Primary（指定主要候选者，加在具体子类上）   @Autowired+@Qualifier("userServiceImpl1")（指定具体名字）   @Resource("AuditOrgaAreaServiceImpl")
`
...
`   ISOLATION_DEFAULT‌： 使用数据库默认的隔离级别。不同的数据库可能有不同的默认隔离级别。`
    ISOLATION_READ_UNCOMMITTED‌（读未提交）：可能会导致 脏读、不可重复读、幻读
    ISOLATION_READ_COMMITTED‌（读已提交）： 可以避免脏读，但可能会出现 不可重复读、幻读
`   ISOLATION_REPEATABLE_READ‌（可重复读，MySQL默认级别）： 可以避免脏读、不可重复读，但可能会出现幻读（Phantom Read），但 MySQL 通过 MVCC + Next-Key Lock 在 RR 级别下已能解决大部分幻读`
...
Spring 什么是循环依赖？
定义：两个或多个 Bean 相互引用对方；
解决方案： `利用 三级缓存机制‌， 解决 ‌单例 Bean 的属性/setter 注入（无法解决构造器注入）‌ 循环依赖`

为什么 Spring 官方却推荐构造器注入？如果遇到构造器循环依赖怎么办？
`   1、final修饰：循环依赖的属性都用final修饰
    2、构造器注入时：其中一个类的构造函数，@Lazy懒加载修饰参数，使用时才创建`