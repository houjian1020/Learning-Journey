...
3、Redis的应用场景(重点)
`   临时数据：滑块校验参数
    缓存：将热点数据放到内存-降低数据库压力，提升并发量
    排行榜‌：利用Redis的有序集合（Sorted Set）数据结构
    计数器：对 String 进行自增自减运算
    夸服务共享: 单点登录中的用户信息 和  Token
    分布式锁：Redisson框架实现`

4、Redis 的持久化机制是什么？各自的优缺点？(重点)
...
29、Redis实现分布式锁(重点)
   Redis实现分布式锁是一种常见的技术，用于在分布式系统中确保某个资源或操作在同一时间只被一个客户端访问或执行
    ...
    实现方式：Redisson框架实现(多数生产环境： Lua 脚本 + Redis 单线程 + Hash（key锁名 Field持锁线程 value重入次数） 结构保证了原子性) /RedLock算法(多redis节点容错) /redis+Lua保证原子性(简单生产环境)
    Redisson推荐：封装`了可重入锁、公平锁、读写锁；内置了看门狗（Watchdog）机制`，彻底解决了“锁过期但业务未执行完”的致命难题
    注意事项：
...
33、什么是redis击穿？(重点)
`   描述：某个 热点Key 在缓存过期瞬间，大量并发 请求直接穿透缓存层访问数据库‌，导致数据库压力骤增甚至崩溃的现象‌
    解决方案：
        Redis分布式锁(强一致性)：仅允许一个线程重建缓存，其他线程等待或重试‌；保证数据的强一致性，但吞吐量下降(使用类似单例模式 的 双层校验，提高效率)
        逻辑过期（高可用性）: 设置 Redis 的物理 TTL，而是在 Value 内部存储一个逻辑过期时间。查询时发现逻辑过期，立即返回旧数据保证可用性，同时启动异步线程去更新缓存。 用户无感知，无需等待，系统吞吐量高；会返回一小段时间的脏数据，不适用于对数据一致性要求极高的场景。`
        

34、什么是redis穿透？(重点)
`   描述：Redis缓存和数据库中都不存在。可能是遭遇外部的 恶意攻击；导致请求直接穿透缓存层，压垮数据库
    解决法案：
        布隆过滤器：项目启动时，创建一个布隆过滤器，将所有可能存在的数据加入过滤器中； 风险：需定期更新过滤器，避免新数据被误判‌
        设置临时key:可以将key-value对写为key-null;  风险：恶意攻击会导致Redis内存占用升高‌
        接口层增加校验：如用户鉴权校验，参数有明显规则约束`
        

35、什么是redis雪崩？(重点)
`   描述：redis宕机/大量缓存数据同时失效，导致mysql的负载过大宕机
    解决方案：
        随机过期时间(最基础有效)：分散Key失效时间，避免集体崩溃‌
        多级缓存架构(高并发场景首选)：优先本地缓Caffeine--->其次Redis--->最后数据库
        redis集群: 负载均衡 故障迁移
        限流与降级(系统保护兜底)：数据库压力过载时主动降级非核心业务‌
`


36、缓存预热
    缓存预热就是系统上线后，将相关的缓存数据直接加载到缓存系统。
    解决方案：初始化Bean时 、 缓存刷新页面、定时任务
...
36、如何保证缓存与数据库双写时的数据一致性？(重点)
`   延迟双删 + 事务回调（最终一致性,高性能）：更新数据库-----删除缓存————重写事物提交后的回调函数——————设置异步延迟删除缓存
    分布式读写锁‌(强一致性，低性能)：更新数据库、删除缓存/读取数据 时，使用分布式锁来确保同一时间只有一个进程或线程可以进行读写操作；
    版本号控制+缓存CAS(乐观锁，中性能)：数据库和缓存中同时存储一个版本号————更新时，比较并更新版本号————读取缓存时，先检查版本号，如果与数据库中的不一致，则重新从数据库加载数据并更新缓存（担心其它操作更新了数据版本）`
...
38、假如Redis里面有1亿个key，其中有10w个key是以某个固定的已知的前缀开头的，如何将它们全部找出来？
