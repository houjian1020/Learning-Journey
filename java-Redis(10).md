...
29、Redis实现分布式锁(重点)
    ...
    实现方式：Redisson框架实现(多数生产环境)/RedLock算法(多redis节点容错)/redis+Lua保证原子性(简单生产环境)
    注意事项：
...
33、什么是redis击穿？(重点)
    描述：某个热点Key在缓存过期瞬间，大量并发请求直接穿透缓存层访问数据库‌，导致数据库压力骤增甚至崩溃的现象‌
    解决方案：
        Redis分布式锁(最优选)：仅允许一个线程重建缓存，其他线程等待或重试‌
        设置永不过期(慎用): 需人工维护热点Key列表，内存可能膨胀‌
        

34、什么是redis穿透？(重点)
    描述：Redis无缓存且数据库也不存在该数据。可能是遭遇外部的恶意攻击
    解决法案：
...
35、什么是redis雪崩？(重点)
    ...
    解决方案：
        多级缓存架构(高并发场景首选)：优先本地缓--->其次Redi--->最后数据库
        redis集群: 负载均衡 故障迁移
        熔断降级(系统保护兜底)：数据库压力过载时主动降级非核心业务‌
...
36、如何保证缓存与数据库双写时的数据一致性？(重点)
    异步延迟双删法‌（最终一致性）：在更新数据库之前，先删除缓存————更新数据库——————睡眠一段时间——————再次删除缓存
    分布式锁‌(强一致性)：更新数据库和缓存时，使用分布式锁来确保同一时间只有一个进程或线程可以进行更新操作
    版本号控制(乐观锁)：数据库和缓存中同时存储一个版本号————更新数据库时，同时更新版本号————读取缓存时，先检查版本号，如果与数据库中的不一致，则重新从数据库加载数据并更新缓存
