...
`14.阻塞与非阻塞本质区别是线程有没有释放（重点）
    阻塞：让出CPU资源，不释放线程
    非阻塞：让出CPU资源，释放线程`

`15.同步与异步、阻塞与非阻塞（重点）
    同步/异步：调用者“主动等待” vs “被动通知”的区别。
    阻塞/非阻塞：调用者线程“原地挂起” vs “继续执行”的区别。`




14、什么是线程死锁？
    死锁是指两个或两个以上的线程在执 行过程中，由于竞争资源或者由于彼此通信而造成的一种阻塞的现象，若无外力作用，它们都将无法推进下去。
    即：线程之间由于资源竞争造成的一种阻塞现象

`15、形成死锁的四个必要条件是什么？(重点)
    互斥： 一个资源仅为一个线程所占有
    不可剥夺： 线程对已经获取的资源未使用完之前不能被剥夺
    保持和请求： 已经持有了一个资源，但是又要访问一个新的被其他线程占用的资源
    循环等待：链中每一个线程程已获得的资源同时被链中下一个进程所请求`
...
`16、如何避免线程死锁？（重点）
    1.破坏形成死锁的四个必要条件:
    2.破坏“循环等待”条件（固定加锁顺序）
    3.破坏“保持和请求”条件（一次性获取所有锁）
    4.破坏“不可剥夺”条件（使用定时锁lock.tryLock(timeout)）`
...
28、你是如何调用 wait() 方法的？使用 if 块还是循环？为什么？
    wait() 方法应该循环调用，因为当线程获取到 CPU 开始执行的时候，其他条件可能还没有满足，所以在处理前，循环检测条件是否满足会更好
    if只会检测一次，而循环会重复检测
...
`35、Java 中 interrupt 和 interrupted 和 isInterrupted 方法的区别？  InterruptExample.java(重点)
    ...
    isInterrupted：非静态方法，查询 中断状态  是true还是false，与interrupt最大的差别
    interrupted：是静态方法，查询并清除 中断状态。如果一个线程被中断了，第一次调用 interrupted 则返回 true，第二次和后面的就返回 false 了。`
...
`37、什么是线程同步，有哪几种实现方式？  SyncExample.java(重点)
    线程同步：解决多线程并发访问共享资源时的“数据不一致”问题，核心加锁
...
`41、在 Java 程序中怎么保证多线程的运行安全？
    方法一：使用原子类、并发集合类（CAS），比如 java.util.concurrent 下的类，使用原子类AtomicInteger
...
48、线程之间如何通信及线程之间如何同步
    线程之间的通信（协作）：通过共享变量来实现；为了确保通信的正确性，需要使用同步机制来保护共享变量。
    线程之间的同步（互斥）：synchronized    ReentrantLock   java.util.concurrent
...
`   synchronized ：是通过监视器（Monitor）来实现的。每个对象都与一个监控器类相关联。
    监视器的结构‌：持有锁的线程标识（threadid）、等待队列（同步等待队列/条件等待队列）、锁重入计数器
...
62、线程 B 怎么知道线程 A 修改了变量（共享变量）
    （1）volatile 修饰变量: 禁止重排序 和 可见性; 不保证复合操作的原子性
    （2）synchronized 、 Lock 锁机制
    （3）wait/notify
    （4）原子类 并发集合类
...
`65、synchronized 和 Lock（ReentrantLock） 有什么区别？(重点)
    ...
    synchronized 写法固定,Lock 提供多种方法，更灵活(公平锁 锁超时)。`
...
70、final不可变对象，它对写并发应用有什么帮助？
    final不可变对象： 将类声明为 final;
                    ...
                    确保所有字段在构造函数中被正确初始化‌;
                    以创建常量的方式创建对象
    帮助：线程安全，适用于并发读场景，多个线程之间共享时不需要进行同步，提升了代码执行效率。


71、Lock 接口和synchronized 对比同步它有什么优势？
`   更灵活：
        （1）支持可中断等待与超时获取锁
        （2）可以使锁更公平，线程等待的时间顺序依次分配锁
        （3）精确唤醒
        （4）跨方法加锁和释放锁`
...
`   ThreadPoolExecutor就是线程池：通过传入不同的参数，构造出适用于不同应用场景下的ThreadPoolExecutor（线程池）
    构造参数参数介绍：
        corePoolSize 核心线程数量: 常驻线程，线程处于空闲状态也不会被销毁
        maximumPoolSize 最大线程数量：当任务队列已满，且当前线程数小于该值时，才会创建非核心（临时）线程来处理任务
        keepAliveTime 空闲存活时间：非核心线程空闲存活时间
        unit 时间单位（比如秒，分）
        workQueue 阻塞队列：缓存等待执行的任务
        threadFactory 线程工厂：创建新线程时，通过工厂设置线程的名称、优先级、是否为守护线程等属性，排查线上问题时非常有用
        handler 线程池拒绝策略：任务队列已满，且线程数已达到 maximumPoolSize 时，直接抛出异常（默认）、回退给提交任务的线程执行、丢弃、丢弃最早的
    阻塞队列：（严谨使用无界队列，防止内存溢出）
           1. ArrayBlockingQueue ：数组组成 的有界阻塞队列（常用：要安全、防 OOM、限流）
           2. LinkedBlockingQueue ：链表组成 的默认无界/有界阻塞队列（常用：要吞吐、异步解耦）
           3. PriorityBlockingQueue ：支持优先级排序 的无界阻塞队列（要插队、VIP 优先）
           4. DelayQueue： 优先级队列实现的 无界阻塞队列（要延时、超时取消）
           5. SynchronousQueue： 不存储元素 的阻塞队列（常用：瞬时高并发任务，且生命周期极短）
           6. LinkedTransferQueue： 链表组成 的无界阻塞队列（要极致低延迟传递）
           7. LinkedBlockingDeque： 链表组成 的双向阻塞队列（要工作窃取、双端操作）`


| 队列类型                     | 底层数据结构            | 容量限制                                | 锁机制                             | 内存开销                                                    | 并发吞吐量                                                                         | 典型应用场景 |
|:-------------------------|:------------------|:------------------------------------|:--------------------------------|:--------------------------------------------------------|:------------------------------------------------------------------------------| :--- |
| 1. ArrayBlockingQueue    | 循环数组              | 强制有界<br>（创建时必须指定）                   | 单锁模型<br>（读写共用一把锁）               | 极低<br>（预分配连续数组，无额外节点对象）                                 | 中等<br>（单锁导致读写互斥，高并发下锁竞争大）                                                     | 核心业务缓冲与限流：如秒杀系统请求缓冲、订单处理队列。强制有界可防止内存溢出（OOM），保护下游服务不被压垮。 |
| 2. LinkedBlockingQueue   | 单向链表              | 可选有界/无界<br>（默认无界 Integer.MAX_VALUE） | 双锁分离<br>（putLock 和 takeLock 独立） | 较高<br>（每个元素需封装为 Node 对象，产生额外内存开销）                       | 高<br>（读写分离，入队出队互不干扰，吞吐量通常比 ABQ 高 15%-20%）                                     | 高吞吐异步任务：如日志收集与异步刷盘、RPC 请求缓冲、后台报表生成。注意：生产环境必须显式指定容量，严禁使用默认无界构造。 |
| 3. PriorityBlockingQueue | 优先级堆<br>（二叉树数组）   | 无界                                  | 单锁模型<br>（单锁 + Condition）        | 中等<br>（堆结构动态扩容，需数组拷贝）                                   | 中等偏低<br>（插入/取出需 O(log n) 调整堆结构，开销较大）                                          | 按优先级调度：如任务调度系统（VIP 客户请求优先处理）、VIP 消息插队、系统资源分配。 |
| 4. DelayQueue            | 优先级堆<br>（内部包装元素）  | 无界                                  | 单锁模型<br>（单锁 + Condition）        | 中等<br>（同 PriorityBlockingQueue）                         | 中等偏低<br>（除了堆操作，还需计算延时，且存在 Leader 线程机制）                                        | 延时任务处理：如电商订单超时未支付自动取消、外卖超时未接单自动取消、定时提醒、缓存过期清理。 |
| 5. SynchronousQueue      | 无存储结构<br>（节点栈/链表） | 容量为 0<br>（不存储元素）                    | 公平/非公平锁                         | 极低<br>（即产即消，无数据堆积）                                      | 极高<br>（无中间缓冲，直接移交，适合高并发瞬时处理）                                                  | 瞬时高并发任务移交：作为 `CachedThreadPool` 的默认队列。适用于生命周期极短、需要快速响应的轻量级异步任务。 |
| 6. LinkedTransferQueue   | 单向链表              | 无界                                  | CAS 无锁算法<br>（减少锁竞争）             | 较高<br>（同链表结构，Node 对象开销）                                 | 极高<br>（支持 transfer 直接传递，吞吐量优于普通链表队列）                                          | 超低延迟数据传递：如高频交易系统中的实时行情推送、高并发消息总线、需要极致响应速度的生产者-消费者模型。 |
| 7. LinkedBlockingDeque   | 双向链表              | 可选有界/无界                             | 双锁分离<br>（putLock/takeLock）      | 较高<br>（双向链表节点开销略大于单向链表）                                 | 高<br>（支持两端操作，适合工作窃取模式，如 Fork/Join）                                            | 工作窃取（Work-Stealing）与双端处理：如 `ForkJoinPool` 的任务队列、浏览器页面渲染任务、支持“从头部插入”的插队或回退机制。 |
...
`80、什么是Executors？  ExecutorsExample.java(重点)
    ...
    建议：通过 ThreadPoolExecutor 手动指定参数来创建线程池，而不是使用 Executors 工具类的快捷方法（如 newFixedThreadPool），因为后者默认使用了无界队列或极大的最大线程数，极易导致内存溢出（OOM）或资源耗尽。`
...
`83、线程池中 ThreadPoolExecutor/ExecutorService的的 submit() 和 ThreadPoolExecutor/ExecutorService的 execute() 方法有什么区别？(重点)
    ...
    不同点：
        ...
        异常：execute()直接打印在控制台，且不会影响其他线程的执行；异常不会立即抛出，而是延迟到调用 future.get() 时，才会以 ExecutionException 的形式抛出`



84、线程池的执行原理？(重点)
    处理流程如图：![img_1.png](img_1.png)
`   判断核心线程：如果当前运行的线程数 < corePoolSize，则立即创建新的核心线程执行该任务。
    尝试入队：如果当前线程数 ≥ corePoolSize，则尝试将任务放入 workQueue 中等待。
    创建临时线程：如果任务入队失败（队列已满），且当前线程数 < maximumPoolSize，则创建非核心（临时）线程来执行该任务。
    触发拒绝策略：如果队列已满，且当前线程数 ≥ maximumPoolSize，则调用 handler 拒绝该任务。`


85、如何合理分配线程池大小?(重点)
`   CPU密集型任务：消耗CPU资源，避免线程频繁切换导致的开销；核心线程数可设为CPU核心数+1；最大线程数 = 核心线程数（或略高于核心线程数）
    IO密集型任务：涉及网络、文件读取等，CPU计算时间少，大部分时间等待I/O操作完成。核心线程数可设为CPU核心数2-4倍；最大线程=核心线程2-4倍`
...
=============================================并发工具类=======================================================
`90、常用的并发工具类有哪些？
    CountDownLatch:主线程等待子线程任务完成。CountDownLatchExample.java
    CyclicBarrier(回环栅栏)：多个线程互相等待，凑齐了一起走。 CyclicBarrierExample.java
    Semaphore (信号量):控制并发访问资源的数量（限流）`
