...
主流微服务组件生态：
API网关：网关是系统的统一入口，`负责路由、鉴权、限流等`。Spring Cloud Gateway 是当前官方推荐的、性能更优的选择
配置中心：配置中心用于`集中管理、动态刷新各服务配置`。Nacos 凭借其易用性和与注册中心的一体化优势，已成为主流
注册中心：注册中心是微服务的“通讯录”，`管理所有服务实例的地址`。Nacos 是目前最主流的选择，可同时作为注册中心和配置中心
负载均衡：负责在客户端将`请求均衡地分发`到多个服务实例。Spring Cloud LoadBalancer 是官方推荐的 Ribbon 替代品
熔断/限流：保障服务`高可用的“保险丝”`，防止故障蔓延。Sentinel 在阿里系及国内企业中被广泛采用，功能强大。
服务调用：服务间的`远程调用方式`。OpenFeign 是声明式的HTTP客户端，使用简便；Dubbo 则是一款高性能的RPC框架，适合内部高频调用
